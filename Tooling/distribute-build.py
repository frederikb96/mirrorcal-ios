...
def group_id() -> str:
    for group in call("GET", f"apps/{APP_ID}/betaGroups").get("data", []):
        if group["attributes"]["name"] == GROUP_NAME:
            return group["id"]
    raise SystemExit(
        f"no beta group named {GROUP_NAME!r} on app {APP_ID} — create it once, by hand or via "
        "the ios skill's asc.py, before the first release: an internal group "
        "(isInternalGroup: true, needs no Apple review), a tester, and an invitation."
    )
