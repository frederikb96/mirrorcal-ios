...
# target, and any package files a future `Package.swift` excludes from the Linux build. Both are
# otherwise unverified until a metered runner compiles them.
#
...
if [ $# -gt 0 ]; then
    ...
    dirs=("$root/MirrorCal" "$root/MirrorCalKit/Sources")
fi
